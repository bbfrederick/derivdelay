...
from derivdelay.filter import NoncausalFilter, getfilterbandfreqs
from derivdelay.io import parsefilespec
from derivdelay.util import version
...
def is_valid_file(parser, arg):
    ...
    """
    if arg is not None:
        thefilename, colspec = parsefilespec(arg)
    else:
        thefilename = None
...
def addfilteropts(
    ...
    filt_opts.add_argument(
        ...
        help=(
            ...
            f'vlf: {getfilterbandfreqs("vlf", asrange=True)}, '
            f'lfo: {getfilterbandfreqs("lfo", asrange=True)}, '
            f'cardiac: {getfilterbandfreqs("cardiac", asrange=True)}, '
            f'hrv_ulf: {getfilterbandfreqs("hrv_ulf", asrange=True)}, '
            f'hrv_vlf: {getfilterbandfreqs("hrv_vlf", asrange=True)}, '
            f'hrv_lf: {getfilterbandfreqs("hrv_lf", asrange=True)}, '
            f'hrv_hf: {getfilterbandfreqs("hrv_hf", asrange=True)}, '
            f'hrv_vhf: {getfilterbandfreqs("hrv_vhf", asrange=True)}, '
            f'lfo_legacy: {getfilterbandfreqs("lfo_legacy", asrange=True)}, '
            f'lfo_tight: {getfilterbandfreqs("lfo_tight", asrange=True)}'
        ),
        default=defaultmethod,
    )
    filt_opts.add_argument(
